...
class FewShotDatasetMapper_stage1:
    ...
    def __call__(self, dataset_dict):
        ...
        # instances = self.instance_2(sem_seg_gt, image_shape)
    ...
    def instance(self, sem_seg_gt, image_shape):
        ...
        # remove sub_val classes
        # classes = classes[~np.isin(classes, self.sub_val_list)]
        instances.gt_classes = torch.tensor(classes, dtype=torch.int64)

        masks = []
        for class_id in classes:
            # if class_id not in self.sub_val_list:
    ...
    def instance_2(self, sem_seg_gt, image_shape):
        sem_seg_gt = sem_seg_gt.numpy()
        instances = Instances(image_shape)
        classes = np.unique(sem_seg_gt)
        # remove ignored region
        ...
        for class_id in classes:
            if class_id not in self.sub_val_list:
                class_mask = sem_seg_gt == class_id
                class_instances = measure.label(class_mask, connectivity=2)
                for i in range(1, class_instances.max()+1):
                    mask = class_instances == i
                    if mask.sum() > 0:
                        masks.append(mask)
        ...
        if len(masks) == 0:
            ...
            instances.gt_masks = masks.tensor
        # print(instances.gt_masks.shape, instances.gt_classes.shape)
        return instances
