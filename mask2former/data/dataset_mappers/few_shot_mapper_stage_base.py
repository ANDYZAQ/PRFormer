...
class FewShotDatasetMapper_stage_base:
    """
    ...
    def __call__(self, dataset_dict):
        ...
        if self.is_train:
            ...
            subcls_list = [self.sub_list.index(cs) for cs in class_chosen]
            # support_image_list, support_label_list, support_label_for_seg_list = self.get_sup_img_pairs(class_chosen)
            label = self.get_label_cats(label, class_chosen, subcls_list)
        else:
    ...
    def get_label_cats(self, label, class_chosen, subcls_list):
        class_chosen = [0, 255] + class_chosen
        label_ = np.zeros_like(label)
        for cls, subcls in zip(class_chosen, subcls_list):
            label_[label == cls] = subcls
        return label_
