...
def load_fewshot_coco_seg(fss_data_list_path, root = None):
    # Read FSS Data
    ...
    for line in f_str:
        ...
        record["file_name"] = os.path.join(root, img)
        record["sem_seg_file_name"] = os.path.join(root, mask.strip())
        record["class_chosen"] = None
...
def load_fewshot_coco_val1000(list_dir, name, split, shot, dataname=None, root=None, fss_sub_class_file_list_path=None):
    assert os.path.isfile(list_dir), list_dir
    ...
    with open(fss_sub_class_file_list_path, 'r') as f:
        f_str = f.read()
    sub_class_file_list = eval(f_str)
