...
with open('lists/coco/coco_name.txt', "r") as f:
...
def register_all_coco(root, split, BASE_CLASS_NAMES, NOVEL_CLASS_NAMES):
    ...
    for name, image_dirname, sem_seg_dirname in [
        ...
        fss_list_root = 'lists/coco/fss_list/{}/'.format(name)
        ...
        if name=="train":
            all_name = f"coco_fewshot_chg_sem_seg_{name}_{str(split)}"
            DatasetCatalog.register(
                all_name,
                # lambda d=d: load_fewshot_voc_seg(d, root),
                lambda d=fss_data_list_path: load_fewshot_coco_seg(d, root = root),
            ...
            shot = 1
            all_name = f"coco_fewshot_chg_sem_seg_{name}_{str(split)}_1shot"
            DatasetCatalog.register(
                all_name,
                lambda d=fss_data_list_path: load_fewshot_coco_val1000(d, name, split = split, shot = shot, dataname = 'coco', root = root, fss_sub_class_file_list_path = fss_sub_class_file_list_path),
                # lambda split=split, name=name: load_fewshot_voc_seg(split, name),
            )
            ...
            shot = 5
            all_name = f"coco_fewshot_chg_sem_seg_{name}_{str(split)}_5shot"
            DatasetCatalog.register(
                all_name,
                lambda d=fss_data_list_path: load_fewshot_coco_val1000(d, name, split = split, shot = shot, dataname = 'coco', root = root, fss_sub_class_file_list_path = fss_sub_class_file_list_path),
                # lambda split=split, name=name: load_fewshot_voc_seg(split, name),
