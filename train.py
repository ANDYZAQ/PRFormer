...
from mask2former.data import (
    ...
    FewShotDatasetMapper_stage1_change, 
    FewShotDatasetMapper_stage2_change, 
    FewShotDatasetMapper_u,
    FewShotDatasetMapper_stage_base,
    build_detection_train_loader,
    build_detection_test_loader,
)
...
def build_train_loader(cfg):
    if cfg.INPUT.DATASET_MAPPER_NAME == "fewshot_sem":
        ...
        mapper = FewShotDatasetMapper_stage2_change(cfg, True)
    elif cfg.INPUT.DATASET_MAPPER_NAME == "fewshot_sem_ori_change":
        mapper = FewShotDatasetMapper_stage1_change(cfg, True)
    elif cfg.INPUT.DATASET_MAPPER_NAME == "fewshot_sem_u":
        mapper = FewShotDatasetMapper_u(cfg, True)
    elif cfg.INPUT.DATASET_MAPPER_NAME == "fewshot_base":
        mapper = FewShotDatasetMapper_stage_base(cfg, True)
    else:
        mapper = None
    return build_detection_train_loader(cfg, mapper=mapper, aspect_ratio_grouping=False)
    ...
    with EventStorage(start_iter) as storage:
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            ...
            if (
                ...
                if results and comm.is_main_process():   
                    ...
                    if cur_mIoU > best_mIoU:
                        ...
                        results_dict = json.dumps(results_dict, indent=4, ensure_ascii=False)
...
def main(args):
    ...
    for dataset_name in cfg.DATASETS.TEST:
        if cfg.INPUT.DATASET_MAPPER_NAME == "fewshot_sem":
            ...
            mapper = FewShotDatasetMapper_u(cfg, False)
        elif cfg.INPUT.DATASET_MAPPER_NAME == "fewshot_sem_ori_change":
            mapper = FewShotDatasetMapper_stage1_change(cfg, False)
        elif cfg.INPUT.DATASET_MAPPER_NAME == "fewshot_base":
            mapper = FewShotDatasetMapper_stage_base(cfg, False)
        else:
            mapper = None
